    ...
    for j, tmp in enumerate(result):
        ...
        if y > datetime.now() - timedelta(hours=24):
            detailed_result_last_day.append({'name': str(k),
                                             ...
                                             'files': int(z),
                                             'size': lib.sizeof_fmt(int(i)),
                                             'status': static_vars.JobStatus[st]})
    b_s_result = lib.gen_chart_array_time_3d(b_s_res)
    b_c_result = lib.gen_chart_array_time_3d(b_c_res)
...
def jobs_report():
    s = """SELECT client.name AS client_name, job.name AS job_name, job.jobstatus AS job_jobstatus,
    ...
    for i, res in enumerate(query_result):
        ...
        result.append({
            ...
            'jstdur': jstdur,
            'jfiles': int(jfiles),
            'jbytes': lib.sizeof_fmt(int(jbytes)),
...
def client_detailed_info(host_name, bdate):
    ...
    for i, _short in enumerate(_short_res):
        ...
        short_res.append({'id': int(j_id),
                          ...
                          'files': int(j_files),
                          'size': lib.sizeof_fmt(int(j_bytes)),
...
def media_report(media):
    ...
    media_info_result['volbytes'] = lib.sizeof_fmt(int(_media_info_result[7]))
